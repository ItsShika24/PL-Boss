...
<head>
    ...
    <style>
        * {
        ...
        /* STYLE BARU UNTUK TOMBOL TAMBAH/KURANG */
        .ticket-counter {
            display: flex;
            align-items: center;
            gap: 15px;
                    ...
                    <form method="POST" action="">
                        ...
                        <div class="form-section">
                            ...
                            <div class="ticket-counter">
                                <button type="button" class="counter-btn minus-btn" id="minusBtn" disabled>−</button>
                                <div class="ticket-display" id="ticketDisplay">0</div>
                                <button type="button" class="counter-btn plus-btn" id="plusBtn">+</button>
                            </div>
    ...
    <script>
        ...
        minusBtn.addEventListener('click', function() {
            if (jumlahTiket > 0) {
                ...
                // Reset kursi jika jumlah tiket berkurang
                if (selectedSeats.length > jumlahTiket) {
                    selectedSeats = selectedSeats.slice(0, jumlahTiket);
                    updateSeatsDisplay();
                    updateSeatsInputs();
                }
        ...
        function updateTicketCounter() {
            ticketDisplay.textContent = jumlahTiket;
            jumlahTiketInput.value = jumlahTiket;
            
            // Update status tombol
            minusBtn.disabled = jumlahTiket === 0;
            plusBtn.disabled = jumlahTiket === MAX_TICKETS;
            
            // Update warna display
            if (jumlahTiket === 0) {
                ticketDisplay.style.color = '#999';
                ticketDisplay.style.background = '#f5f5f5';
            } else {
                ticketDisplay.style.color = '#667eea';
                ticketDisplay.style.background = '#f8f9ff';
            }
        }
        ...
        function toggleSeat(seatId, seatCode, element) {
            ...
            if (selectedSeats.length >= jumlahTiket && !selectedSeats.find(s => s.id === seatId)) {
                alert(`Anda hanya dapat memilih ${jumlahTiket} kursi`);
                return;
            }
        ...
        function updateSummary() {
            const totalPrice = jumlahTiket * ticketPrice;

            // Update jumlah tiket di ringkasan
            document.getElementById('summaryTiket').textContent = jumlahTiket;
        ...
        function updateSubmitButton() {
            ...
            if (jumlahTiket > 0 && selectedSeats.length === jumlahTiket) {
                submitBtn.innerHTML = `🎟️ Pesan ${jumlahTiket} Tiket - Rp ${totalPrice.toLocaleString('id-ID')}`;
                submitBtn.style.background = 'linear-gradient(135deg, #4caf50 0%, #45a049 100%)';
                submitBtn.style.color = 'white';
            } else {
                submitBtn.innerHTML = '🎟️ Konfirmasi Pemesanan';
        ...
        function checkFormValidity() {
            ...
            const tiketSelected = jumlahTiket > 0;
            const seatsSelected = selectedSeats.length > 0;
            const seatsMatchTiket = selectedSeats.length === jumlahTiket;
